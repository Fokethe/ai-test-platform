# AI Test Platform - Documentation Index  
  
> Last Updated: 2026-03-04

---

## 📁 文档目录

```
docs/
├── README.md                          # 文档总览（本文档）
├── INDEX.md                           # 快速索引
├── 99-历史归档/                        # 📦 历史报告归档
│   ├── BUGHUNTER_REPORT.md            # BugHunter 检测报告
│   ├── BUGHUNTER_FIX_REPORT.md        # BugHunter 修复报告
│   ├── HEALTH_CHECK_REPORT.md         # 健康检查报告
│   ├── PROJECT_CLEANUP_REPORT.md      # 项目清理报告
│   ├── PROJECT_FIX_REPORT.md          # 项目修复报告
│   ├── PROJECT_FINAL_STATUS.md        # 项目最终状态
│   ├── FINAL_FIX_COMPLETE.md          # 最终修复完成报告
│   └── kimi.batch-*.md                  # 批次处理记录
│
├── 01-架构文档/                        # 架构设计
├── 02-API文档/                         # API 接口文档
├── 03-部署配置/                        # 部署相关配置
├── 04-使用指南/                        # 用户指南
├── 05-参考资料/                        # 参考资料 (PRD已移至根目录)
└── 06-项目管理/                        # 项目进度管理
```

---

## 🚀 快速导航

### 项目报告
| 报告 | 位置 | 说明 |
|------|------|------|
| BugHunter 检测 | [99-历史归档/BUGHUNTER_REPORT.md](./99-历史归档/BUGHUNTER_REPORT.md) | 深度代码诊断 |
| BugHunter 修复 | [99-历史归档/BUGHUNTER_FIX_REPORT.md](./99-历史归档/BUGHUNTER_FIX_REPORT.md) | 修复执行记录 |
| 健康检查 | [99-历史归档/HEALTH_CHECK_REPORT.md](./99-历史归档/HEALTH_CHECK_REPORT.md) | 系统健康度检查 |
| 项目清理 | [99-历史归档/PROJECT_CLEANUP_REPORT.md](./99-历史归档/PROJECT_CLEANUP_REPORT.md) | 清理执行报告 |

### 使用指南
| 文档 | 位置 | 说明 |
|------|------|------|
| **系统账号** | [04-使用指南/系统账号.md](./04-使用指南/系统账号.md) | 内置测试账号密码 |
| **KIMI 指南** | [04-使用指南/KIMI.md](./04-使用指南/KIMI.md) | AI 协作规范 |
| **CLAUDE 指南** | [04-使用指南/CLAUDE.md](./04-使用指南/CLAUDE.md) | Claude 使用指南 |
| **Vibe Coding** | [04-使用指南/Kimi_Code_Vibe_Coding_Guide.md](./04-使用指南/Kimi_Code_Vibe_Coding_Guide.md) | AI 辅助编程 |

### 核心文档
| 文档 | 位置 | 说明 |
|------|------|------|
| **📋 PRD 产品需求** | [../PRD.md](../PRD.md) | **核心产品文档**，功能需求与路线图 |
| **🔧 KIMI 开发指南** | [04-使用指南/KIMI.md](./04-使用指南/KIMI.md) | 技术规范与开发约定 |
| **📝 项目进度** | [06-项目管理/progress.md](./06-项目管理/progress.md) | 当前进度与里程碑 |

### 角色指南
| 角色 | 推荐文档 |
|------|---------|
| **产品经理** | [📋 PRD](../PRD.md) |
| **开发人员** | [KIMI.md](./04-使用指南/KIMI.md), [CLAUDE.md](./04-使用指南/CLAUDE.md) |
| **测试人员** | [系统账号.md](./04-使用指南/系统账号.md) |
| **AI 辅助** | [Vibe Coding Guide](./04-使用指南/Vibe_Coding_Guide.md) |

---

**最后更新**: 2026-03-04
