...
- [ ] Harden workspace/project governance APIs and permission boundaries (AC: 1, 2)
  - [ ] Ensure `/api/projects` list queries are constrained by workspace membership of `session.user.id` (not only query params).
  - [ ] Ensure `/api/projects/[id]` read/update/delete checks membership and role before returning or mutating data.
  - [ ] Add/complete project member assignment flow (list/add/remove) aligned with existing workspace membership model.
- [ ] Enforce resource-level visibility for project metadata and assets (AC: 2, 3)
  - [ ] Add project-scope authorization checks for `/api/systems`, `/api/systems/[id]`, and `/api/pages`.
  - [ ] Validate resource ownership chains (`workspace -> project -> system -> page`) before read/write operations.
  - [ ] Return consistent `401/403/404` responses via unified API response utilities.
- [ ] Complete project-level metadata management path (AC: 1, 3)
  - [ ] Ensure system/page creation paths persist required metadata tied to valid project/system IDs.
  - [ ] Add/update validations for required fields and malformed payloads via shared request parsing helpers.
  - [ ] Keep write paths auditable and ready for future FR6/FR8 role-management extension.
- [ ] Align dashboard workflows with backend requirements (AC: 1, 3)
  - [ ] Fix project creation UI flow to provide required `workspaceId` when calling `/api/projects`.
  - [ ] Ensure workspace/project detail pages display permission-aware actions and stable member/project counts.
- [ ] Add test coverage for governance behavior and regressions (AC: 1, 2, 3)
  - [ ] Add route tests for workspace/project/system/page APIs: unauthorized, forbidden, authorized, and not-found paths.
  - [ ] Add regression tests to prevent cross-workspace/project data leakage.
  - [ ] Update relevant UI tests for project creation and permission-driven rendering.
